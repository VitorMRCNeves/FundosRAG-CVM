...
from utils.DagLaminaFI.task_transferencia_bronze_silver_rentab_ano.schema import (
...
def fn_transferencia_bronze_silver_rentab_ano(data_interval_end: str) -> None:
    """
    Publica snapshot rentab_ano Bronze → Silver.
    ...
    df = DeltaTable(f"s3://{BUCKET_BRONZE}/fundos/rentab/rentab_ano").to_pandas(
    ...
    df_final = apply_schema(df_final, schema=SCHEMA_SILVER_RENTAB_ANO)

    path = f"s3://{BUCKET_SILVER}/fundos/rentab/rentab_ano"
    write_deltalake(
        path,
        df_final,
