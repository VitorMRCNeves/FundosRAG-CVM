...
from utils.DagLaminaFI.task_transferencia_bronze_silver_carteira.schema import (
...
def fn_transferencia_bronze_silver_carteira(data_interval_end: str) -> None:
    """
    Publica snapshot carteira Bronze → Silver.
    ...
    df = DeltaTable(f"s3://{BUCKET_BRONZE}/fundos/laminas/carteira/").to_pandas(
    ...
    df_final = apply_schema(df_final, schema=SCHEMA_SILVER_CARTEIRA)

    path = f"s3://{BUCKET_SILVER}/fundos/laminas/carteira/"
    write_deltalake(
        path,
        df_final,
