...
from utils.DagLaminaFI.task_transferencia_bronze_silver_rentab_mes.schema import (
...
def fn_transferencia_bronze_silver_rentab_mes(data_interval_end: str) -> None:
    """
    Publica snapshot rentab_mes Bronze → Silver.
    ...
    df = DeltaTable(f"s3://{BUCKET_BRONZE}/fundos/rentab/rentab_mes").to_pandas(
    ...
    df_final = apply_schema(df_final, schema=SCHEMA_SILVER_RENTAB_MES)

    path = f"s3://{BUCKET_SILVER}/fundos/rentab/rentab_mes"
    write_deltalake(
        path,
        df_final,
