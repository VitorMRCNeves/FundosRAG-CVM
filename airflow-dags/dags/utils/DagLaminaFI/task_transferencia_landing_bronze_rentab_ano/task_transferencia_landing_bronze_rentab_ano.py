...
from utils.DagLaminaFI.task_transferencia_landing_bronze_rentab_ano.schema import (
...
def fn_transferencia_landing_bronze_rentab_ano(data_interval_end: str) -> None:
    """
    Lê o CSV da tabela rentab_ano na Landing, aplica schema Bronze e persiste em Delta Lake (append).
    ...
    df = pd.read_csv(
        f"s3://{BUCKET_LANDING}/fundos/laminas/rentab_ano/ano={ano}/mes={mes}/rentab_ano_{data_str}.csv",
        sep=",",
        decimal=".",
        dtype=str,
    )
    ...
    df = apply_schema(
        df=df, schema=SCHEMA_BRONZE_RENTAB_ANO, rename_cols=True, select_cols=True
    )

    path = f"s3://{BUCKET_BRONZE}/fundos/rentab/rentab_ano"
    write_deltalake(
        path,
        df,
